...
void ServoMotorImpl::resetPosition() {
  this->pos = 0;
...
void ServoMotorImpl::moveServoTo180SelfTest() {
  if (pos < 180 && activeServo) {
    Serial.println("180 self test moving");
    activeServo = false;                       // tell servo to go to position in variable 'pos'
    ...
    startServoTo180SelfTest = false;
    servoFinish = true;
    Serial.println("fine 180");
  }
}
